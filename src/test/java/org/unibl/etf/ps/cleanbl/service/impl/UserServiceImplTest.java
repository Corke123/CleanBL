...
class UserServiceImplTest {
    ...
    void getAuthorities_should_return_a_list_of_authorities() {
        ...
        List<String> stringList = userSpring.getAuthorities()
                .stream()
    ...
    void getUserByUsername_should_return_user() {
        User user = User.builder().id(1L).username("marko123").email("marko123@gmail.com").build();
    ...
    void getDepartmentOfficers_should_return_all_department_officers() {
        Role role = Role.builder().name("DepartmentOfficer").build();

        List<User> users = Collections.singletonList(
                User.builder().id(1L).username("user123").email("user123@gmail.com").build());
